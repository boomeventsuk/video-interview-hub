...
<body style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;background:#f8fafc;margin:0;padding:24px;">
  <div style="max-width:720px;margin:0 auto;background:#ffffff;border:1px solid #e5e7eb;border-radius:12px;padding:24px;">
    <h1 style="margin:0 0 12px;font-size:22px;color:#111827;">New Boombastic Event Assistant video interview</h1>
    ...
    <p style="margin:20px 0 0;color:#6b7280;font-size:13px;">Videos are hosted in the InterviewPro Supabase video bucket under the Boombastic Event Assistant folder.</p>
  </div>
</body>
</html>`;
}
...
export default function EventAssistantInterview() {
  ...
  const uploadAnswer = async (answer: Answer, sessionId: string) => {
    const filePath = `boombastic-event-assistant/${sessionId}/${answer.questionId}.${answer.ext}`;
    ...
    try {
      ...
      const { data, error } = await supabase.functions.invoke("send-email", {
        body: {
          ...
          subject: `Event Assistant video interview - ${name}`,
          html,
          templateType: "share",
        },
      });
    ...
    <main className="min-h-screen bg-[#09090b] text-white">
      <div className="mx-auto flex min-h-screen w-full max-w-5xl flex-col px-5 py-6 sm:px-8">
        <header className="flex items-center justify-between">
          <div>
            <p className="text-xs uppercase tracking-[0.28em] text-fuchsia-300">Boombastic Events</p>
            <p className="mt-1 text-sm text-zinc-400">Event Assistant video interview</p>
          </div>
            ...
            {stage === "welcome" && (
              <div className="max-w-3xl">
                ...
                <h1 className="max-w-3xl text-4xl font-black leading-tight sm:text-6xl">
                  Casual Event Assistant work in Northampton and surrounding areas.
                </h1>
              ...
              <div className="mx-auto max-w-2xl rounded-xl border border-white/10 bg-white/[0.04] p-6 text-center">
                {submitted ? (
                  <>
                    ...
                    <p className="mt-2 text-zinc-400">Thanks, {name}. Your video answers have been sent to Boombastic Events.</p>
                  </>
