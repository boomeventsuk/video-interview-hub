...
interface PersistenceConfig {
  templateId: string;
  questionDbIds: Record<string, string>;
  warningLabel: string;
}
...
export interface DirectInterviewConfig {
  ...
  heroTitle: string;
  heroCopy: string;
  questions: DirectInterviewQuestion[];
  storageFolder: string;
  notifyEmail: string;
  notifyName: string;
  emailHeading: string;
  emailSubjectPrefix: string;
  emailStorageLabel: string;
  completeSentText: (name: string) => string;
      ...
      return `
        <tr>
          <td style="padding:12px;border-bottom:1px solid #e5e7eb;vertical-align:top;">
            <strong>${escapeHtml(question?.label || `Question ${index + 1}`)}</strong><br/>
            <span style="color:#4b5563;">${escapeHtml(question?.text || "")}</span>
          </td>
          <td style="padding:12px;border-bottom:1px solid #e5e7eb;vertical-align:top;">
            <a href="${escapeHtml(answer.url)}" style="color:#2563eb;">Watch video</a>
          </td>
        </tr>`;
    })
    .join("");
...
<body style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif;background:#f8fafc;margin:0;padding:24px;">
  <div style="max-width:720px;margin:0 auto;background:#ffffff;border:1px solid #e5e7eb;border-radius:12px;padding:24px;">
    ...
    <table style="width:100%;border-collapse:collapse;margin-bottom:20px;">
      ...
      ${phone ? `<tr><td style="padding:6px 0;color:#6b7280;">Phone</td><td style="padding:6px 0;">${escapeHtml(phone)}</td></tr>` : ""}
    </table>
...
export default function DirectInterviewPage({ config }: { config: DirectInterviewConfig }) {
  ...
  const progress = Math.round(((questionIndex + (stage === "review" || pendingAnswer ? 1 : 0)) / config.questions.length) * 100);
  ...
  const startRecording = (index = questionIndex) => {
    ...
    recorder.onstop = () => {
      ...
      setPendingAnswer({ questionId: question.id, blob, ext: mimeInfo.ext });
      setStage("review");
    };
    recorder.start();
  ...
  const uploadAnswer = async (answer: Answer, sessionId: string) => {
    const filePath = `${config.storageFolder}/${sessionId}/${answer.questionId}.${answer.ext}`;
  ...
  const createSubmissionRecord = async () => {
    ...
    const { data, error } = await supabase
      .from("submissions")
      .insert({
        template_id: config.persistence.templateId,
        applicant_name: name,
    ...
    if (error || !data) {
      console.warn(`${config.persistence.warningLabel} submission record was not created. Continuing with storage and email notification.`, error);
      return null;
    }
  ...
  const saveAnswerRecords = async (submissionId: string, uploadedAnswers: Answer[]) => {
    ...
    const rows = uploadedAnswers
      .map((answer) => ({
        ...
        question_id: config.persistence?.questionDbIds[answer.questionId],
    ...
    if (error) {
      console.warn(`${config.persistence.warningLabel} answer records were not created. Video files and email links are still available.`, error);
    }
  };
    ...
    try {
      ...
      const { data, error } = await supabase.functions.invoke("send-email", {
        body: {
          ...
          toName: config.notifyName,
          subject: `${config.emailSubjectPrefix} - ${name}`,
          html,
          templateType: "share",
        },
      });
      ...
      <div className="mx-auto flex min-h-screen w-full max-w-5xl flex-col px-5 py-6 sm:px-8">
        <header className="flex items-center justify-between">
          ...
          <div className="hidden items-center gap-2 text-sm text-zinc-400 sm:flex">
            ...
            {config.questions.length} questions, 1 minute each
          </div>
        </header>
                ...
                <div className="rounded-xl border border-white/10 bg-white/[0.04] p-6">
                  <div className="flex items-center justify-between">
                    <span className="rounded-full bg-white/10 px-3 py-1 text-xs font-semibold text-zinc-300">
                      {currentQuestion.label} of {config.questions.length}
                    </span>
                  ...
                  <div className="mt-6 flex gap-3">
                    {stage === "prep" ? (
                      <Button onClick={() => { clearTimer(); startRecording(questionIndex); }} className="bg-white text-zinc-950 hover:bg-zinc-200">
                        Start recording now
                      </Button>
                    ) : (
